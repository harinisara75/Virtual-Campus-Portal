...
export default function Timetable({ user }) {
  ...
  useEffect(()=>{
    API.get(`/timetable/${date}`).then(r=>setPeriods(r.data.periods || [])).catch(()=>setPeriods([]));
  },[date]);
  ...
  const save = async () => {
    ...
    try { await API.post('/timetable', { date, periods }); alert('Saved'); } catch(e){ console.error(e); alert('Save failed'); }
  };
