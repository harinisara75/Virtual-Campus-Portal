...
export default function Notices({ user }) {
  ...
  const load = async () => {
    try { const r = await API.get('/notices'); setList(r.data); } catch(e){ console.error(e); }
  };

  const add = async () => {
    if (!user || (user.role!=='teacher' && user.role!=='admin')) { alert('Only teachers can post'); return; }
    try {
      const r = await API.post('/notices', { title, message });
  ...
  const remove = async (id) => {
    ...
    try { await API.delete(`/notices/${id}`); setList(prev => prev.filter(x=>x._id !== id)); } catch(e){console.error(e); alert('Delete failed');}
  };
